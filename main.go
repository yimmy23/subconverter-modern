...
func isSupportedTarget(target string) bool {
	...
	case "mihomo", "singbox", "uri", "v2rayng", "surge", "surfboard", "loon", "quanx":
		return true
	default:
		return false
	}
...
func isDNSGeneralKey(key string) bool {
	...
	case "dns-server", "doh-server", "encrypted-dns-server", "encrypted-dns-follow-outbound-mode",
		...
		"ipv6", "proxy-test-url", "internet-test-url", "test-timeout":
		return true
	default:
		return false
	}
...
func parseProxyGroup(spec string, names []string) proxyGroup {
	...
	for _, token := range parts[2:] {
		...
		case numericCSV(token):
			...
			if len(values) >= 2 {
				group.Timeout = parseInt(values[1], 5)
			}
			if len(values) >= 3 {
	...
	if group.Timeout == 0 && (group.Type == "url-test" || group.Type == "fallback" || group.Type == "load-balance") {
		group.Timeout = 5
	}
	if len(group.Proxies) == 0 {
...
func mihomoDNS(template dnsTemplate) dnsConfig {
	...
	encryptedServers := filterDNSResolvers(encryptedDNSValues(template), true)
...
func renderTarget(target string, proxies []map[string]any, parsed parsedTemplate, publicBaseURL string) (renderResult, error) {
	...
	case "surfboard":
		body := []byte(renderSurfboardConfig(proxies, parsed))
		return renderResult{Body: body, ContentType: "text/plain; charset=utf-8", Extension: "conf", Renderer: "subconverter-modern/surfboard"}, nil
	case "loon":
...
func renderSurfboardConfig(proxies []map[string]any, parsed parsedTemplate) string {
	var b strings.Builder
	b.WriteString("# Generated by subconverter-modern for Surfboard\n[General]\n")
	b.WriteString("ipv6 = ")
	b.WriteString(firstNonEmpty(strings.ToLower(parsed.DNS.General["ipv6"]), "false"))
	b.WriteByte('\n')
	if value := parsed.DNS.General["dns-server"]; value != "" {
		b.WriteString("dns-server = ")
		b.WriteString(value)
		b.WriteByte('\n')
	}
	if values := surfboardDoHServers(parsed.DNS); len(values) > 0 {
		b.WriteString("doh-server = ")
		b.WriteString(strings.Join(values, ", "))
		b.WriteByte('\n')
	}
	for _, key := range []string{"always-real-ip", "skip-proxy"} {
		if value := parsed.DNS.General[key]; value != "" {
			b.WriteString(key)
			b.WriteString(" = ")
			b.WriteString(value)
			b.WriteByte('\n')
	...
	b.WriteString("proxy-test-url = ")
	b.WriteString(firstNonEmpty(parsed.DNS.General["proxy-test-url"], defaultTestURL))
	b.WriteByte('\n')
	if value := parsed.DNS.General["internet-test-url"]; value != "" {
		b.WriteString("internet-test-url = ")
		b.WriteString(value)
		b.WriteByte('\n')
	}
	b.WriteString("test-timeout = ")
	b.WriteString(firstNonEmpty(parsed.DNS.General["test-timeout"], "5"))
	b.WriteByte('\n')

	b.WriteString("\n[Proxy]\n")
	for _, proxy := range proxies {
		if line := surfboardProxyLine(proxy); line != "" {
			b.WriteString(line)
			b.WriteByte('\n')
		}
	}

	knownPolicies := map[string]bool{"DIRECT": true, "REJECT": true}
	for _, proxy := range proxies {
		if name := stringValue(proxy["name"]); name != "" && surfboardProxyLine(proxy) != "" {
			knownPolicies[name] = true
		}
	}
	for _, group := range parsed.Groups {
		knownPolicies[group.Name] = true
	}
	b.WriteString("\n[Proxy Group]\n")
	for _, group := range parsed.Groups {
		group.Proxies = filterKnownTags(group.Proxies, knownPolicies)
		if len(group.Proxies) == 0 {
			group.Proxies = []string{"DIRECT"}
		}
		b.WriteString(surfboardGroupLine(group))
		b.WriteByte('\n')
	}
...
func surfboardDoHServers(template dnsTemplate) []string {
	return filterLoonDOHServers(encryptedDNSValues(template))
}

func encryptedDNSValues(template dnsTemplate) []string {
	values := append(csvValues(template.General["doh-server"]), csvValues(template.General["encrypted-dns-server"])...)
	return uniqueStrings(values)
}

func surfboardProxyLine(proxy map[string]any) string {
...
func surfboardGroupLine(group proxyGroup) string {
	parts := append([]string{group.Name + " = " + surgeGroupType(group.Type)}, group.Proxies...)
	if group.Type == "url-test" || group.Type == "fallback" {
		parts = append(parts,
			"url="+firstNonEmpty(group.URL, defaultTestURL),
			"interval="+strconv.Itoa(maxInt(group.Interval, 300)),
			"timeout="+strconv.Itoa(maxInt(group.Timeout, 5)),
		)
		if group.Type == "url-test" {
			parts = append(parts, "tolerance="+strconv.Itoa(maxInt(group.Tolerance, 100)))
		}
	}
	return strings.Join(parts, ", ")
}
...
func surgeDNSGeneralLines(template dnsTemplate) []string {
	...
	for _, key := range []string{
		"dns-server", "encrypted-dns-follow-outbound-mode",
		"encrypted-dns-skip-cert-verification", "hijack-dns", "allow-dns-svcb",
		"use-local-host-item-for-proxy", "always-real-ip", "skip-proxy", "bypass-tun",
	} {
	...
	if encrypted := encryptedDNSValues(template); len(encrypted) > 0 {
		out = append(out, "encrypted-dns-server = "+strings.Join(encrypted, ", "))
	}
	return out
}
...
func loonDNSGeneralLines(template dnsTemplate) []string {
	...
	if encrypted := filterLoonDOHServers(encryptedDNSValues(template)); len(encrypted) > 0 {
...
func quantumultXDNSLines(template dnsTemplate) []string {
	var out []string
	if encrypted := filterDNSResolvers(encryptedDNSValues(template), true); len(encrypted) > 0 {
		var doh []string
		var doq []string
